...
/**
 * App creates an HTTP Server to handle requests
 */
public class App {
    ...
    public static void main(String[] args) {
        ...
        Spark.get("/", (req, res) -> {
            res.type("text/html");
            return App.class.getResourceAsStream("/web/index.html");
        ...
        // GET route that returns all proposals and likes. All we do is get
        ...
        Spark.get("/proposals/:id", (request, response) -> {
        ...
        Spark.post("/proposals", (request, response) -> {
            ...
            int newId = db.insertRow(req.proposalText, req.numLikes);
            if (newId == -1) {
        ...
        /**
         * PUT route for adding/removing a like for a post.
         * 
         * The route has a variable liking that is either true or false to represent
         * if the post id is being liked or unliked
         * 
         */
        Spark.put("/proposals/:id/:liking", (request, response) -> {
            ...
            if (isLiking.equals("true")){
                result = db.updateOne(idx,1);
            }
            else { //should be false
                result = db.updateOne(idx,0);
            }
